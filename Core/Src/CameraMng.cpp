...
short CameraMng::SCCB_Write(uint8_t reg_addr, uint8_t data) {
...
short CameraMng::SCCB_Read(uint8_t reg_addr, uint8_t *pdata) {
	short opertionStatus = 0;
	HAL_StatusTypeDef connectionStatus;
	__disable_irq();
...
CameraMng::CameraMng(CameraCfg cfg) : m_hdcmi(cfg.hdcmi), m_hdma_dcmi(cfg.hdma_dcmi), m_hi2c2(cfg.hi2c2)
{

}

void CameraMng::initialization()
{
...
void CameraMng::setRegistersConfiguration(const std::vector<std::pair<uint8_t, uint8_t>>& registerCfg)
{
...
void CameraMng::configuration(CameraResolution resolution)
{
...
void CameraMng::captureScreenshot()
{
...
void CameraMng::startContinuousCapture()
{

}

void CameraMng::stopContinuousCapture()
{
